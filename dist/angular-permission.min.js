...
!function () {
  ...
  function b(a, b, c, d, e, f, g) {
    a.$on("$stateChangeStart", function (a, b, h, i, j, k) {
      function l() {
        d.toState = b, d.toParams = h, d.fromState = i, d.fromParams = j, d.options = k
      }

      function m(a) {
        angular.extend(b, {$$isAuthorizationFinished: a})
      }

      function n() {
        return b.$$isAuthorizationFinished
      }

      function o() {
        e.broadcastStateChangePermissionAccepted(), d.options = angular.extend({}, d.options, {notify: !1}), c.go(d.toState.name, d.toParams, angular.extend({}, d.options, {location: "replace"})).then(function () {
          e.broadcastStateChangeSuccess()
        })
      }

      function p(a, b) {
        e.broadcastStateChangePermissionDenied(), b.resolveRedirectState(a).then(function (a) {
          c.go(a.state, a.params, a.options)
        })
      }

      if (!n() && (a.preventDefault(), m(!0), l(), !e.areStateEventsDefaultPrevented())) {
        e.broadcastStateChangePermissionStart();
        var q = new g;
        f.authorize(q).then(function () {
          o()
        })["catch"](function (a) {
          p(a, q)
        })["finally"](function () {
  ...
  a.$inject = ["$stateProvider"], b.$inject = ["$rootScope", "$location", "$state", "TransitionProperties", "TransitionEvents", "StateAuthorization", "StatePermissionMap"], angular.module("permission", ["ui.router"]).config(a).run(b)
}(), function () {
  "use strict";
  function a(a) {
    function b(b) {
